/* This C Model for ATTINY series was developed by ASHUTOSH JHA (eSim Winter Intern - 2020)

				****!!! LATEST EDIT - 9:05 PM, 8/7/2020 by AJ ****!!! 

	1. Added timer1
	2. Added new timer0 modes - FPWM, PCPWM, CTC and Normal (FPWM previously added)
	3. Added functionality to check execution time for entire simulation
	4. Added functionality to check total execution time taken by C model for simulation
...
void timer0(int flag)
{
// TIMER0 FPWM by AJ 03/05/20 
// PCPWM by SM, CTC by SK and Normal mode by VY
...
void timer1(int flag)		//TIMER 1 OPERATION BY SK   	 02/06/2020
{    

    unsigned char temp = 0x0; static unsigned int count=0;   					 
...
//	Modified by SM 	11/5/20
	else if(b1==0x1 && b2>=4 && b2<=7)
	{
	    ...
	    unsigned char r=((b2>>1)&1)*16+b4, d=(b2&1)*16+b3;
...
//	RETI by SK 		 on 13/5/2020
	...
	{	
		...
		PC = (SRAM[SP-1].data << 4 | SRAM[SP].data);
	}
...
//	ROR by SK 13/05/2020
	...
	{	int t;GPR[b3+16].data=0x55;
		...
		if(GPR[b3+16].data == 0x0)			//updating Z flag
			SREG[1].data = 1;
		else
			SREG[1].data = 0;

		if(GPR[b3+16].data>=0x08)			//updating N flag
			SREG[2].data = 1;
		else  	
			SREG[2].data = 0;
...
//	INC BY SK		11/05/2020
	else if(b1==0x09 && (b2==0x05 || b2==0x04) && b4==0x03)
	{
		...
		if(GPR[b3+16].data==0x7F)			//updating V flag
			SREG[3].data = 1;
		else  	
			SREG[3].data = 0;

		GPR[b3+16].data += 0x01;			//incrementing data
...
/************************************************************************************************/
//	CALL BY SK		12/05/2020
	else if(b1==0x9 && (b2==0x4 || b2==0x5) && b4==0x2)
	{	
		int temp;
		if(debugMode == 1)
			{
				printf("\nCALL instruction decoded\n");
				printf("\nBefore execution: Reg[%d]: %X",b3+16,GPR[b3+16].data);
			}
		temp=GPR[b3+16].data;
		GPR[b3+16].data=((temp & 0x0F)<<4) | ((temp &0xF0)>>4);
...
void output(int flag)			//Function to compute output for current instruction
{
	if(flag == 1)
	{
		...
		if(debugMode==1)
			{
				PrintSREG();
				printf("Execution time till now (in sec): %f\n",total_Time);
			}
		clk_Count++;
	}
}
