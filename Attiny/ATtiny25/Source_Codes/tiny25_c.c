/* This C Code for ATTINY series  was developed by ASHUTOSH JHA
  Modified by Shubangi Mahajan for ATtiny25
   ...
   **NOTE	:-	The functions "MapToRam" and "output" &
   				...
   				linked to the VHDL code of ATTINY25
   				by "ghdl_access.vhdl" file.	*/ 

   // CODE WITH COMMENTS AND TIMER0(all 4 modes) and TIMER 1.

#include<stdio.h>
#include<math.h>
#include<string.h>
#define size 1024		//1kb program memory for attiny25

int debugMode=1;
int PB0,PB1,PB2,PB3,PB4,PB5,wait_Clocks=0;
int PC = 0;                      // changed PC to int - 10 bit
int SP = 128;                	//IOREG[29]+IOREG[30]*256;
struct memory					//Structure to store RAM and other registers
{
	unsigned char data;
}prog_mem[size],GPR[32],SREG[8],IOREG[64],SRAM[128];
...
/************************************************************************************************/
//	ANDI by AJ		02/05/20
//	Modified by SM 		18/06/20
	else if(b1==0x07)
	{
		unsigned char k = b2*16 + b4;
		unsigned char d = b3+16;
		if(debugMode == 1)
		{
...
//  OR by VY           12-05-2020
    else if(b1==0x02 && b2>=0X08 && b2<=0X0B)
    {
        ...
        if(GPR[d].data == 0x0)//changing carry falg
            SREG[1].data = 1;
        else
            SREG[1].data = 0;
...
//	ST by SM 12/06/2020
	...
	{
		unsigned char r= (b2 & 1)*16+b3;				//x is the value loaded in LDI instruction
		int x=26;
		if(debugMode==1)
		{	
		...
		if(b4==0xC)
			GPR[x].data=GPR[r].data;						//For normal ST
		else if(b4==0xD)
		{	
			printf("\nST+ instruction decoded\n");
			GPR[x].data=GPR[r].data;						//For normal ST+
